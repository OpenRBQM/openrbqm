...
openrbqm_attach <- function() {
  ...
  # openrbqm with the package version on the right-hand side
  load_header <- cli::rule(
    left = crayon::bold("Attaching packages"),
    right = paste0("openrbqm ", package_version("openrbqm"))
  )

  # Return a character string containing the package version for each of openrbqm's constituents
...
openrbqm_detach <- function() {
  pak <- paste0("package:", c(pkgs, "openrbqm"))
